...
export interface WorkerAutomationSettingsDocument { key: 'default'; autoAnalyzeJobs: boolean; autoPrepareApplications: boolean; minScoreToPrepare: number; minScoreToAssistant: number; requireHumanReviewBeforeSubmit: boolean; blockedKeywords: string[]; preferredKeywords: string[]; }
...
const schema = new Schema<WorkerAutomationSettingsDocument>({
  ...
  minScoreToPrepare: { type: Number, required: true }, minScoreToAssistant: { type: Number, required: true }, requireHumanReviewBeforeSubmit: { type: Boolean, required: true },
