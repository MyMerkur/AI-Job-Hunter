...
interface WorkerGeneratedCVDocument { cvProfileId: Types.ObjectId; jobId?: Types.ObjectId; content: string; coverLetterContent: string; provider?: AIProviderName; filePath?: string; format: 'text' | 'markdown' | 'pdf' | 'docx'; status: GeneratedCvStatus; }
...
const generatedCvSchema = new Schema<WorkerGeneratedCVDocument>({
  ...
  provider: String, filePath: String, format: { type: String, required: true }, status: { type: String, required: true },
